...
import com.weibo.toil.ui.helper.OnItemMoveListener;
...
public class ChannelAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements OnItemMoveListener {
    ...
    private ArrayList<Constant.Channel> mSavedChannel, mOtherChannelItems;
    private Context mContext;
    private ItemTouchHelper mItemTouchHelper;

    public ChannelAdapter(Context context, ItemTouchHelper itemTouchHelper, ArrayList<Constant.Channel> savedChannel, ArrayList<Constant.Channel> otherChannelItems) {
    ...
    public void onItemMove(int fromPosition, int toPosition) {
        if (fromPosition < mSavedChannel.size() + 1 && toPosition < mSavedChannel.size() + 1 && toPosition > 0) {
            //显示栏目内移动
            Constant.Channel item = mSavedChannel.get(fromPosition - 1);
            ...
            //未显示栏目移动到显示栏目
            Constant.Channel item = mOtherChannelItems.get(fromPosition - 2 - mSavedChannel.size());
            ...
            //未显示栏目内移动
            Constant.Channel item = mOtherChannelItems.get(fromPosition - 1 - mSavedChannel.size() - 1);
